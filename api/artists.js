...
artistsRouter.get('/', (req, res, next) =>{
    db.all('SELECT * FROM Artist WHERE Artists.is_currently_employed = 1',
    (err, artists) =>{
        if(err){
            next(err);
        }else{
...
artitstRouter.get('/:artistId', (req, res, next)=>{
...
artistsRouter.put('/:artistId', (res, req, next)=>{
    const name = req.body.artist.name;
    const dateOfBirth = req.body.artist.dateOfBirth;
    const biography = req.body.artist.biography;
    const isCurrentlyEmployed = req.body.artist.isCurrentlyEmployed === 0 ? 0 : 1;
    ...
    'biography = $biography, is_currenlty_employed = $isCurrentlyEmployed ' +
